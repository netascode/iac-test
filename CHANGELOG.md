...
- pyats broker: disconnect all devices concurrently during broker shutdown via `asyncio.gather`, reducing post-test teardown from O(n × disconnect_time) to O(max(disconnect_time)). Measured 210s → 20s on a 21-device fleet.
